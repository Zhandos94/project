 ...
 */
class AucData extends \yii\db\ActiveRecord
{
	public $lotTypeId;
	...
	{
		return [
			[['source_auc_id', 'cruser_id', 'org_id', 'subject_id', 'lienor_id', 'agent_id', 'colcat_id', 'buy_status', 'sale_type', 'auction_type', 'lastupdateuserid', 'count_lots', 'region_id', 'is_collateral', 'is_direct_sale', 'is_operator_ads'], 'integer'],
			...
			[['agent_phone', 'agent_email', 'agent_address', 'pledgor', 'payment_details', 'margin_pay_cond', 'notifications', 'media_public', 'title_ru', 'title_kz', 'imgname', 'imgsavename', 'venue'], 'string', 'max' => 255],
			[['lotTypeId'], 'integer'],
			[['name_ru', 'name_kz', 'lotTypeId'], 'required'],
		];
	}
	...
	{
		$lot = Lot::find()->where(['auc_id' => $this->id])->one();
		if ($lot === null) {
			$lot = new Lot();
		}
		$lot->auc_id = $this->id;
		$lot->user_id = Yii::$app->user->id;
		$lot->location = 'qwe';//TODO place to location
		$lot->lot_type_id = $this->lotTypeId;
		if($lot->save()){
			return $lot;
		} else {
			Yii::error($lot->getErrors(), 'lot_data');
		}
		return false;
	}
	...
	{
		return Lot::find()->where(['auc_id' => $this->id])->one()->lot_type_id;
	}
}
